...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        FileOutputStream fileOutputStream = new FileOutputStream("basilisk-launcher.jar");
        ...
        if (System.getProperty("os.name").contains("Windows")) {
            ...
            for (File jars : HOME.listFiles()) {
                if (jars.getName().contains("basilisk-launcher")) {
                    download = false;
                }
            }
            if (download) {
                downloadUsingNIO("https://github.com/BasiliskPlugins/basilisk-launcher/releases/download/v0.0.1/basilisk-launcher-0.0.1.jar", System.getProperty("user.home") + "\\.runelite\\sideloaded-plugins\\basilisk-launcher-0.0.1.jar");
            }
        ...
        if (args.length != 0) {
            ...
            if (isUnix()) {
                ...
                try {
                    ...
                    fileOutputStream = new FileOutputStream(args[0] + "/squashfs-root/basilisk-launcher-0.0.1.jar");
        ...
        try {
            ...
            if (System.getProperty("os.name").contains("Windows")) {
                if (args.length != 0) {
                    fileOutputStream = new FileOutputStream(args[0] + "\\basilisk-installer.jar");
                } else {
                    fileOutputStream = new FileOutputStream(System.getProperty("user.home") + "\\AppData\\Local\\RuneLite\\basilisk-installer.jar");
                }
            }
            if (System.getProperty("os.name").contains("Mac OS X")) {
                if (args.length != 0) {
                    fileOutputStream = new FileOutputStream(args[0] + "/Contents/Resources/basilisk-installer.jar");
                } else {
                    fileOutputStream = new FileOutputStream("/Applications/RuneLite.app/Contents/Resources/basilisk-installer.jar");
                }
            }
            ...
            object.append("classPath", "basilisk-installer.jar");
        ...
        try {
            if (args.length != 0) {
                if (isUnix()) {
                    //pack appimage
                    System.out.println(args[0] + "/appimagetool-x86_64.AppImage " + args[0] + "/squashfs-root" + " basilisk-installer.AppImage");
                    Process extractionProcess = Runtime.getRuntime().exec(args[0] + "/appimagetool-x86_64.AppImage " + args[0] + "/squashfs-root" + " basilisk-installer.AppImage");
                    extractionProcess.waitFor();
                }
            }
        } catch (Exception e) {
